...
5. 复杂度分级：按 `tiny` / `small` / `medium` / `large` / `greenfield` 控制文档、规范、多代理和 `/goal` 的重量。
6. 执行：按场景路由到 debugging、planning、UI/UX、TDD、subagents 或官方 `/goal`。
...
- 创建或审查 `AGENTS.md`、`CLAUDE.md`、`memory/`：读 `references/rules-and-memory.md`。
- 选择 OpenSpec 或 Spec-Kit：读 `references/spec-layer.md`。
- 不确定该用哪个 skill：读 `references/skill-routing.md`。
- 长任务、官方 `/goal`、自我迭代、续跑：读 `references/goal-task-loop.md`。
...
- medium / large / greenfield 未完成深聊对齐，不允许 `/goal`、多代理、Spec-Kit implement 或 OpenSpec apply。
- 同一任务不能同时使用 OpenSpec 和 Spec-Kit。
- 小 bug、小文案、小配置修改不要创建 PRD、Spec、`/goal`、多代理或完整 run ledger。
